...
// Prototypes
void init(); //clean, called by main
void ascii_art(); //seems clean, just prints the file
void meet_santa(); // seems clean, deals with the names
void* calloc_check(size_t, size_t); // clean
unsigned long get_int(); // clean
void print_santa_options(); // clean
void ask_santa(child*); // clean
void add_wish(child*);
...
// Global variables
// probably this inside .bss
// 16 kids, each of them with their own wishes and letters and names
child *children[MAX_CHILDREN] = {0};
unsigned int children_count = 0; //counter for the kids
...
void ascii_art()
{
    ...
    length = ftell(file);
    buffer = calloc(1, length + 1); // probably not a fastbin
    if (!buffer)
    {
        fclose(file);
...
// just an utility function for the calloc
...
void meet_santa()
{
    ...
    while (1)
    {
		// main cycle breaks when all kids have dealt with santa
        ...
        len = 0;
        // name at least 4 bytes
		while (len <= 3)
        {
            memset(name, 0, NAME_SIZE);
            printf("Name: ");
            len = read(STDIN_FILENO, name, NAME_SIZE-1); // no overflow
            if (len <= 3)
        ...
        {
            if (strcmp(name, children[i]->name) == 0) //if the kid is already in the db
            {
                puts("Oooh! I know you, little one! Did you forget to ask Santa a present?");
                ask_santa(children[i]); // next function, proceed if recognized
                new_child = false;
            }
        }
        if (new_child)
        {
            children[i] = calloc_check(1, sizeof(child)); // alloc
            children[children_count]->name = strdup(name); // copy of the name, returns a pointer SAVED IN THE HEAP, fastbin..
            printf("Nice to meet you %s! Do you want to write me a letter for Christmas?\n", children[children_count]->name);
            ask_santa(children[children_count]); //proceed
            children_count++; //only after we update children_count..
        }
    }
...
// reads strings and converts them in base10 numbers..
